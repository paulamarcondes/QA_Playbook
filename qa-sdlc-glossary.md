...
## Team Practices, Tools & Documentation

- **QA Guild**: A recurring community where QAs from different teams share practices, tools, learnings, standards, and improvement ideas.
- **Three Amigos**: A collaboration session where Product, Development, and QA align on behavior, risks, and acceptance criteria before implementation.
- **SonarQube**: A static analysis platform commonly used to identify code quality, maintainability, security, duplication, and coverage issues.
- **Static Code Analysis**: Automated inspection of code without executing it, usually to find quality, security, maintainability, or style issues.
- **IFD**: Interface Design Document. A document that describes how systems exchange data, including flow, format, rules, dependencies, and validation expectations.
- **ICD**: Interface Control Document. A contract-style document that defines interface details, data structures, formats, versions, and responsibilities between systems.
- **User Guide**: Documentation that helps users understand how to use a feature or workflow.
- **How-to-Test Notes**: Lightweight technical notes explaining how to validate a feature, including setup, data, environment, steps, expected logs, and troubleshooting tips.
- **QA Report**: A concise report or presentation that communicates testing scope, results, risks, defects, release recommendation, and improvement actions.
- **QA Assessment Survey**: A lightweight survey used to understand team quality maturity, culture, tooling, and improvement opportunities.
- **Nielsen Heuristics**: A set of usability principles used to evaluate whether a user interface is clear, consistent, recoverable, and easy to use.
- **CLI**: Command Line Interface. A text-based way to run commands, scripts, tests, searches, and file operations efficiently.
- **MCP**: Model Context Protocol. A way for AI tools to connect with external systems, such as issue trackers, documentation tools, repositories, or test management tools.
